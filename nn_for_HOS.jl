...
function process_data_padded_tensors(number_of_samples::Int, pad_value::Float32=0.0f0)
    ...
    for ii in 1:number_of_samples
        ...
        if !isfile(input_timeseries_path)
            error("File not found: $input_timeseries_path. Please ensure all timeseries files exist.")
        end
        ...
        if !isfile(input_parameters_path)
            error("File not found: $input_parameters_path. Please ensure all parameters files exist.")
        end
        ...
        println("  Sample $ii: X_size $(size(time_and_eta)), Y_size $(size(parameters))")
    end
...
y_reshaped = dropdims(y_labels, dims=2)
...
const device = Metal.functional() ? gpu : cpu
...
output_dim = size(y_train_gpu, 1) # (3)
